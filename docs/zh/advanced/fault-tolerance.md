...
- Rollout dump：训练时自动写入 `{--save}/rollout_dumps/rollout_{rollout_id}/`（评估为 `rollout_eval_{rollout_id}/`），可检查或 replay；
- `--load-debug-rollout-data /path/to/rollout_{rollout_id}`：加载已保存 rollout data（分片目录或旧版 `.pt`），并跳过 SGLang 初始化；
