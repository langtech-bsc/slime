...
训练时 rollout dump 会写入 `{--save}/rollout_dumps/rollout_{rollout_id}/`；其中 sample trace 包含 SGLang `meta_info` 里的 timing 字段。trace viewer 直接读取这些 attrs，并用 `pd_*` 字段展示 `[P]` / `[D]` 虚拟 lane。
