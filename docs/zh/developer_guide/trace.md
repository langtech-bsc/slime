...
训练时 rollout dump **默认开启**，写入 `{--save}/rollout_dumps/rollout_{rollout_id}/`（约 128 MiB 分片 + `manifest.json`）。评估 rollout 写入 `rollout_eval_{rollout_id}/`。每个 part 文件包含 rollout samples 及对应 `trace` 数据。也可用 `--load-debug-rollout-data /path/to/rollout_{rollout_id}` 复用同一份 dump。
